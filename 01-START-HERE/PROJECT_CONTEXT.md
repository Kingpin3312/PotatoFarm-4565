...
**76 database models · 62 enums · 27 API routers · 166 procedures ·
44 screens · 27 scheduled jobs · 23 audit scripts · 45 check suites.**
...
```bash
npm test                    # 314 assertions, no database, ~3 seconds
```
...
**Unit tests cover the pure logic, not the codebase.** 314 assertions in 18 files,
  ...
  stateful is still covered only by the 45 check suites and the
