...
**81 database models · 67 enums · 36 API routers · 213 procedures ·
46 screens · 29 scheduled jobs · 23 audit scripts · 58 check suites.**
...
```bash
npm test                    # 410 assertions, no database, ~3 seconds
```
...
27 test files, and the selection is not "whatever was easy to test". Every
...
**Unit tests cover the pure logic, not the codebase.** 410 assertions in 27 files,
  ...
  stateful is still covered only by the 58 check suites and the
...
**Database:** PostgreSQL via Prisma. `app/prisma/schema.prisma`, 81
