...
**75 database models · 62 enums · 27 API routers · 159 procedures ·
44 screens · 27 scheduled jobs · 23 audit scripts · 40 check suites.**
...
```bash
npm test                    # 310 assertions, no database, ~3 seconds
```
...
17 test files, and the selection is not "whatever was easy to test". Every
...
| `server/lib/plans/run.test.ts` | Every nurture step ends with a person doing something, and each waits its own delay |
...
**Unit tests cover the pure logic, not the codebase.** 310 assertions in 17 files,
  ...
  stateful is still covered only by the 40 check suites and the
