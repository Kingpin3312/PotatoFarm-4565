...
| B11 | P1 | Buyer requirements are never captured | Only voice intake creates a `Requirement`; the demo brokerage has none, so matching and "buyers in <area>" find nobody | Agents record what a buyer wants on the person page; the assistant's extraction adds areas and bedrooms; seed gives demo buyers requirements | `routers/requirements.ts`, person page, `assistant/extract.ts`, seed | Medium | `check:requirements` | OPEN |
...
| C3 | P2 | "buyers in dubai marina" returns 0 | Diagnosed: the query is read correctly; no buyer in the demo brokerage has a requirement, because nothing but voice intake ever writes one. See B11 | — | — | — | — | → B11 |
